...
'body { font-family: Inter, sans-serif; background: #060F1E; color: white;' +
'  display: flex; align-items: center; justify-content: center;' +
'  height: 100vh; flex-direction: column; gap: 16px; }' +
'.icon { font-size: 48px; }' +
'h1 { font-size: 22px; font-weight: 700; }' +
'p { font-size: 14px; color: rgba(255,255,255,0.45); text-align: center; max-width: 340px; line-height: 1.6; }' +
'.badge { background: rgba(248,113,113,0.1); border: 1px solid rgba(248,113,113,0.3);' +
'  color: #FCA5A5; padding: 6px 16px; border-radius: 20px; font-size: 12px; font-weight: 600; }' +
'</style></head><body>' +
'<div class="icon">\uD83D\uDD12</div>' +
...
// ─────────────────────────────────────────────────────
// DASHBOARD HTML — uses string concat, NO template literals
// ─────────────────────────────────────────────────────
function getDashboardHTML(recordId, objectName, recordName, userName) {
  ...
  // Escape for safe JS injection into the page script
  var safeObjectName = JSON.stringify(objectName);
  var safeRecordName = JSON.stringify(recordName);
...
'<script>' +
'var oN=' + safeObjectName + ', rN=' + safeRecordName + ', uN=' + safeUserName + ';' +
'var D={' +
'  Account:{' +
'    ctx:"Account: "+rN,' +
...
'  }' +
'};' +
'var data=D[oN]||D.Account;' +
'var ini=uN.split(" ").map(function(n){return n[0];}).join("").toUpperCase().substr(0,2);' +
...
'document.getElementById("tl").innerHTML=data.tl.map(function(t,i){' +
'  return "<div class=\\"tl-item\\" style=\\"animation-delay:"+(0.05*i)+"s\\"><div class=\\"tl-dot "+t.d+"\\"></div><div><div class=\\"tl-t\\">"+t.t+"</div><div class=\\"tl-m\\">"+t.m+"</div></div></div>";' +
'}).join("");' +
'document.getElementById("ins").innerHTML=data.ins.map(function(x,i){' +
'  return "<div class=\\"ins-card\\" style=\\"animation-delay:"+(0.1+0.05*i)+"s\\"><div class=\\"ins-icon\\">"+x.i+"</div><div><div class=\\"ins-title\\">"+x.t+"</div><div class=\\"ins-desc\\">"+x.d+"</div><span class=\\"ins-tag "+x.c+"\\">"+x.l+"</span></div></div>";' +
'}).join("");' +
'document.getElementById("tasks").innerHTML=data.tasks.map(function(t,i){' +
'  return "<div class=\\"task\\" style=\\"animation-delay:"+(0.1+0.05*i)+"s\\"><div class=\\"tcheck "+(t.done?"done":"")+"\\">"+( t.done?"\u2713":"")+"</div><div class=\\"ttext "+(t.done?"done":"")+"\\">"+t.t+"</div><div class=\\"tdue "+(t.ov?"ov":"")+"\\">"+t.due+"</div></div>";' +
'}).join("");' +
'setTimeout(function(){var t=document.getElementById("toast");t.style.transition="opacity .3s";t.style.opacity="0";setTimeout(function(){t.remove();},300);},4000);' +
'<\/script>' +
'</body></html>';
...
app.post('/canvas', (req, res) => {
  ...
  if (!signedRequest) {
    console.log('❌ No signed_request in body. Body:', req.body);
  ...
  try {
    ...
    console.log('✅ Signature verified! User:', ctx.context && ctx.context.user && ctx.context.user.fullName);
 
    res.send(getDashboardHTML(
      ctx.parameters && ctx.parameters.recordId,
      ctx.parameters && ctx.parameters.objectName,
      ctx.parameters && ctx.parameters.recordName,
      ctx.context && ctx.context.user && ctx.context.user.fullName
    ));
