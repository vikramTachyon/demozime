...
app.post('/canvas', (req, res) => {
  ...
  try {
    ...
    const sig     = parts[0];           // ✅ FIXED: keep as string, not Buffer
    ...
    // ✅ FIXED: digest as base64 string for direct comparison
    ...
    // ✅ FIXED: simple string comparison (timingSafeEqual was causing buffer mismatch)
    if (sig !== expected) {
      console.log('❌ Signature mismatch');
      return res.status(403).send(getBlockedHTML());
    }
    const ctx = JSON.parse(Buffer.from(payload, 'base64').toString('utf8'));
    console.log('✅ Signature verified! User:', ctx.context?.user?.fullName);
